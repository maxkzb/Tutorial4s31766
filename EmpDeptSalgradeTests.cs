...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(emp => emp.Job == "DEPT 30").OrderByDescending(emp => emp.Sal).ToList(); 
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var ChicagoDept = depts.Where(dept => dept.Loc == "Chicago").Select(dept => dept.DeptNo);
        
        List<Emp> result = emps.Where(emp => ChicagoDept.Contains(emp.DeptNo)).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = (from emp in emps
            ...
            where emp.Sal >= s.Losal && emp.Sal <= s.Hisal
                select new { EName = emp.EName, Grade = s.Losal }).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(g => new { DeptNo = g.Key, AvgSal = g.Average(e => e.Sal) })
            .ToList();
